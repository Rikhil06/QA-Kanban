...
const { data } = useQuery<{
  ...
  queryFn: async () => {
    const res = await fetch(`https://qa-backend-105l.onrender.com /api/site/${slug}?teamId=${user?.teamId}`, {
  ...
  const mutation = useMutation({
    mutationFn: ({ reportId, newStatus }: { reportId: string; newStatus: ColumnId }) =>
      updateStatus(token, reportId, newStatus),
    onMutate: async ({ reportId, newStatus }) => {
      await queryClient.cancelQueries({ queryKey: ['reports', slug, user?.teamId] });
    ...
    onError: (_err, _variables, context) => {
      if (context?.previousData) {
        queryClient.setQueryData(['reports', slug, user?.teamId], context.previousData);
      }
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['reports', slug, user?.teamId] });
    },
  });
  ...
  const filteredReports = reports.filter((report: Report) => {
    ...
    if (filters.assignee.length > 0 && !filters.assignee.includes(report.userName)) return false;
    if (filters.pages.length > 0 && !filters.pages.includes(report.pagePath === '/' ? 'home' : report.pagePath.replace(/^\/|\/$/g, ""))) return false;
    return true;
  });
  ...
  const columns = useMemo(() => {
    ...
    return {
      new: { name: 'New', items: sorted.filter(r => r.status === 'new'), columnColour: [{ bgColour: 'bg-sky-100', accentColour: 'border-l-sky-500' }] },
      inProgress: { name: 'In Progress', items: sorted.filter(r => r.status === 'inProgress'), columnColour: [{ bgColour: 'bg-orange-100', accentColour: 'border-l-orange-500' }] },
      done: { name: 'Done', items: sorted.filter(r => r.status === 'done'), columnColour: [{ bgColour: 'bg-indigo-100', accentColour: 'border-l-indigo-500' }] },
    };
  ...
  const handleReportDeleted = (deletedId: string) => {
    queryClient.setQueryData<{ reports: Report[]; comments: Record<string, Comment[]> }>(
      ['reports', slug, user?.teamId],
      (oldData) => {
        if (!oldData) return oldData;
        return { ...oldData, reports: oldData.reports.filter(r => r.id !== deletedId) };
      }
    );
    setSelectedId(null);
  };
  ...
  return (
    <>
      <FilterBar filters={filters} setFilters={setFilters} onClearFilters={() => setFilters({ status: [], priority: [], assignee: [], pages: [] })} users={users} pages={uniquePages} />
      <div className='flex h-[calc(100vh-7.5rem)] gap-4 overflow-x-auto px-6 py-6'>
        <DragDropContext onDragEnd={onDragEnd}>
          {Object.entries(columns).map(([id, column]) => (
            <Droppable droppableId={id} key={id}>
              {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className='flex min-w-[320px] flex-col rounded-xl border transition-colors border-white/6 bg-[#1C1C1C]/40'>
                            ...
                            >
                              <div className="mb-2 flex items-start justify-between gap-2">
                                ...
                                <div className="flex gap-1">
                                  <span className="rounded bg-white/6 px-1.5 py-0.5 text-[10px] text-white/50">{Capitalize(item.type)}</span>
                                </div>
                              </div>

                              <h4 className="mb-1.5 text-sm text-white/90 group-hover:text-white">{item.title}</h4>
                              {item.comment && <p className="mb-3 text-xs text-white/40 line-clamp-2">{item.comment}</p>}
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex h-6 w-6 items-center justify-center rounded-full bg-linear-to-br from-violet-500 to-purple-600 text-[10px] ring-2 ring-[#222] transition-transform group-hover:scale-110" title={item.userName}>
