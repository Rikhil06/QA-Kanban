        ...
        <div className="flex-1 max-w-2xl">
            <div className="relative group">
                ...
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    <kbd className="px-1.5 py-0.5 text-xs bg-white/10 rounded border border-white/10 text-gray-500">⌘</kbd>
                    <kbd className="px-1.5 py-0.5 text-xs bg-white/10 rounded border border-white/10 text-gray-500">K</kbd>
                </div>
            </div>

            {loading && <p className="absolute mt-2 text-gray-400 text-sm">Searching...</p>}
            ...
            {query && !loading && !error && (
                <div className="absolute mt-2 w-full bg-[#0A0A0A] border border-white/10 rounded-lg shadow-lg max-h-80 overflow-y-auto z-50">
                    {results.projects.length > 0 && (
                        <div>
                        <p className="px-3 py-1 text-gray-400 text-xs uppercase">Projects</p>
                        {results.projects.map((p: any) => (
                            <div key={p.id} className="px-3 py-2 hover:bg-white/10 cursor-pointer">
                            {p.name}
                            </div>
                    ))}
                 </div>
                ...
                {results.issues.length > 0 && (
                    <div>
                    <p className="px-3 py-1 text-gray-400 text-xs uppercase">Issues</p>
                    {results.issues.map((i: any) => (
                        <div key={i.id} className="px-3 py-2 hover:bg-white/10 cursor-pointer">
                ...
                {results.users.length > 0 && (
                    ...
                    {results.users.map((u: any) => (
                        <div key={u.id} className="px-3 py-2 hover:bg-white/10 cursor-pointer">
                        {u.name} ({u.email})
                        </div>
                    ))}
                    </div>
