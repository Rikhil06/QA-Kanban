...
export function UsersTasks() {
    ...
    const { data: tasks, isLoading } = useSWR(token ? ['http://127.0.0.1:4000/api/users-tasks', token] : null, ([url, token]) => fetcher(url, token));
  ...
  return (
    <div className="bg-linear-to-br from-[#1A1A1A] to-[#161616] rounded-xl border border-white/10 p-6 shadow-2xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="text-sm text-purple-400 hover:text-purple-300 transition-colors cursor-pointer font-semibold">
          View all
        </button>
      </div>
